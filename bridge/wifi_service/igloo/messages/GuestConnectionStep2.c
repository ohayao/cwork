#include "GuestConnectionStep2.h"
#include "../cbor/cbor.h"

void ig_GuestConnectionStep2_init(IgGuestConnectionStep2 *obj)
{
  memset(obj, 0, sizeof(IgGuestConnectionStep2));
}
IgSerializerError ig_GuestConnectionStep2_encode(IgGuestConnectionStep2 *obj,uint8_t *retval,uint32_t length,size_t *written_length)
{
  ...
  if(!ig_GuestConnectionStep2_is_valid(obj)) return IgSerializerErrorInvalidPayload;
      ...
      switch (tag_id) {
        case 0:
          if(value_type == CborIntegerType){
            ...
            //msgId value
            if(val != 10) return IgSerializerErrorInvalidMsgIdValue;
            err = cbor_value_advance_fixed(&content);
            break;
          }
        ...
        case 11:
            if(value_type == CborByteStringType){
                ...
                ig_GuestConnectionStep2_set_nonce_nocopy(retval, data_arr, size);
                break;
            }
          default:
