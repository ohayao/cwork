...
fsm_table_t admin_unpair_fsm_table[ADMIN_UNPAIR_SM_TABLE_LEN] = {
  {BLE_ADMIN_UNPAIR_BEGIN,         register_admin_notfication,   BLE_ADMIN_UNPAIR_STEP1},
  {BLE_ADMIN_UNPAIR_STEP1,         waiting_admin_step1,          BLE_ADMIN_UNPAIR_STEP2},
  {BLE_ADMIN_UNPAIR_STEP2,         write_admin_step2,            BLE_ADMIN_UNPAIR_ESTABLISHED},
  {BLE_ADMIN_UNPAIR_ESTABLISHED,   write_unpair_request,         BLE_ADMIN_UNPAIR_RESULT},
  {BLE_ADMIN_UNPAIR_RESULT, waiting_unpair_result,        BLE_ADMIN_UNPAIR_DONE},
};

typedef struct AdminUnpairConnection {
	gatt_connection_t* gatt_connection;
  igm_lock_t lock;
	enum BLE_ADMIN_UNPAIR_STATE admin_unpair_step;
	size_t step_max_size;
	size_t  step_cur_size;
	size_t n_size_byte;
	uint8_t *step_data;
  uuid_t admin_uuid;
  int has_admin_unpair_result;
  ble_admin_unpair_result_t *admin_unpair_result;
}admin_unpair_connection_t;
...
int save_message_data(const uint8_t* data, int data_length, void* user_data)
{
  ...
  admin_unpair_connection_t *admin_unpair_connection = 
                              (admin_unpair_connection_t *)ble_data->ble_connection;
  if (admin_unpair_connection->step_max_size == 0)
  {
    if (data_length<3)
    {
    ...
    else
    {
      if (data[2] == 0xff)
      {
        admin_unpair_connection->n_size_byte = 3;
        admin_unpair_connection->step_max_size = data[0] * (0xfe) + data[1] + 3;
        serverLog(LL_NOTICE, 
                      "2 bytes lenth %d", admin_unpair_connection->step_max_size);
      }
      else
      {
        admin_unpair_connection->n_size_byte = 1;
        admin_unpair_connection->step_max_size = data[0] + 1;
        serverLog(LL_NOTICE, 
                      "1 bytes lenth %d", admin_unpair_connection->step_max_size);
      }
      admin_unpair_connection->step_cur_size = 0;
      admin_unpair_connection->step_data = (uint8_t *)malloc(
                                            admin_unpair_connection->step_max_size);
      if (!(admin_unpair_connection->step_data))
      {
  ...
  int size_left = 
        admin_unpair_connection->step_max_size - admin_unpair_connection->step_cur_size;

  if (size_left < data_length)
	{
    serverLog(LL_NOTICE, "size_left < data_length");
    admin_unpair_connection->step_max_size += 85;
		uint8_t *old_data = admin_unpair_connection->step_data;
		admin_unpair_connection->step_data = (uint8_t *)malloc(
                                              admin_unpair_connection->step_max_size);
		if (!admin_unpair_connection->step_data)
		{
		...
		memcpy(
      admin_unpair_connection->step_data, old_data, admin_unpair_connection->step_cur_size);
		free(old_data);
	}
  ...
  // 空间足够, 直接放下空间里面
	for (int j = 0; j < data_length; ) {
		// printf("%02x ", data[i]);
		admin_unpair_connection->step_data[admin_unpair_connection->step_cur_size++] = data[j++];
	}
}
...
int write_admin_step2(void *arg)
{
  ...
  ble_data_t *ble_data = task_node->ble_data;
  ble_admin_unpair_param_t *param = ble_data->ble_param;
  admin_unpair_connection_t *admin_unpair_connection = 
                              (admin_unpair_connection_t *)ble_data->ble_connection;
  ...
  connectionID = ret;
  setLockConnectionID(&(admin_unpair_connection->lock), connectionID);
  serverLog(LL_NOTICE, "set connectionID to Lock");

  step2Len = igloohome_ble_lock_crypto_AdminConnection_genConnStep2Native(
		connectionID, 
    admin_unpair_connection->step_data + admin_unpair_connection->n_size_byte, 
  ...
  admin_unpair_connection->admin_unpair_step = BLE_ADMIN_UNPAIR_STEP2;
  
  admin_unpair_connection->step_max_size = 0;
  admin_unpair_connection->n_size_byte = 0;
  admin_unpair_connection->step_cur_size = 0;
  free(admin_unpair_connection->step_data);
  admin_unpair_connection->step_data = NULL;
  free(payloadBytes);
  free(step2Bytes);
...
static int write_unpair_request(void *arg)
{
  ...
  ig_UnpairRequest_set_password(
    &unpair_request, param->lock->password, param->lock->password_size);
  ...
  admin_unpair_connection->admin_unpair_step = BLE_ADMIN_UNPAIR_REQUEST;
  return 0;
...
static int handle_unpair_responce(const uint8_t* data, int data_length,void* user_data)
{
  ...
  if (admin_unpair_connection->step_max_size == admin_unpair_connection->step_cur_size)
  {
    ...
    admin_unpair_connection->admin_unpair_step = BLE_ADMIN_UNPAIR_RESULT;
    size_t messageLen = 
      admin_unpair_connection->step_max_size-admin_unpair_connection->n_size_byte;
    uint8_t messageBytes[messageLen];
    memcpy(messageBytes, 
      admin_unpair_connection->step_data+admin_unpair_connection->n_size_byte, messageLen);
	...
	const uuid_t* uuid, const uint8_t* data, size_t data_length, void* user_data)
{
  ...
  switch(admin_unpair_connection->admin_unpair_step)
  {
    case BLE_ADMIN_UNPAIR_BEGIN:
    {
      serverLog(LL_NOTICE, 
      "admin_unpair_connection->admin_unpair_step BLE_ADMIN_BEGIN handle_step1_message");
      handle_step1_message(data, data_length,user_data);
      break;
    }
    case BLE_ADMIN_UNPAIR_STEP2:
    {
      serverLog(LL_NOTICE, 
      "admin_unpair_connection->admin_unpair_step BLE_PAIRING_STEP3 handle_step3_message");
    ...
    default:
    {
      serverLog(LL_ERROR, "admin_unpair_connection->admin_unpair_step error");
      break;
    }
  }
}
...
int handle_step3_message(const uint8_t* data, int data_length,void* user_data)
{
  ...
  if (admin_unpair_connection->step_max_size == admin_unpair_connection->step_cur_size)
  {
    int ret;
    serverLog(LL_NOTICE, "handle_step2_message RECV step2 data finished");
    admin_unpair_connection->admin_unpair_step = BLE_ADMIN_UNPAIR_ESTABLISHED;

    int rec_ret = igloohome_ble_lock_crypto_AdminConnection_recConnStep3Native(
			(admin_unpair_connection->lock).connectionID, 
      admin_unpair_connection->step_data + admin_unpair_connection->n_size_byte, 
      admin_unpair_connection->step_max_size-admin_unpair_connection->n_size_byte
		);
...
int handle_step1_message(const uint8_t* data, int data_length,void* user_data)
{
  ...
  if (admin_unpair_connection->step_max_size == admin_unpair_connection->step_cur_size)
  {
    int ret;
    serverLog(LL_NOTICE, "handle_step2_message RECV step2 data finished");
    admin_unpair_connection->admin_unpair_step = BLE_ADMIN_UNPAIR_STEP1;
    g_main_loop_quit(task_node->loop);
  }
}
...
int register_admin_notfication(void *arg)
{
  ...
  ble_admin_unpair_param_t *param = (ble_admin_unpair_param_t *)(ble_data->ble_param);
  // 分配 connection, 传递到其他函数的数据,
  ble_data->ble_connection = calloc(sizeof(admin_unpair_connection_t), 1);
  admin_unpair_connection_t *admin_unpair_connection = 
                            (admin_unpair_connection_t *)ble_data->ble_connection;
  memset(admin_unpair_connection, 0, sizeof(admin_unpair_connection_t));

  admin_unpair_connection->has_admin_unpair_result = 1;
  admin_unpair_connection->admin_unpair_result = calloc(sizeof(ble_admin_unpair_result_t), 1);
  bleInitAdminUnpairResult(admin_unpair_connection->admin_unpair_result);
  bleSetAdminUnpairResultAddr(admin_unpair_connection->admin_unpair_result, 
  ...
  serverLog(LL_NOTICE, "register_admin_notfication ready to connection %s",
                                                            param->lock->addr);
  admin_unpair_connection->gatt_connection = gattlib_connect(
    NULL, param->lock->addr, GATTLIB_CONNECTION_OPTIONS_LEGACY_DEFAULT);
  if (admin_unpair_connection->gatt_connection == NULL) {
  ...
  serverLog(LL_NOTICE, "Succeeded to connect to the bluetooth device." );
   ...
   if (
    gattlib_string_to_uuid(
      admin_str, strlen(admin_str), &(admin_unpair_connection->admin_uuid))<0)
  {
  ...
  gattlib_register_notification(
    admin_unpair_connection->gatt_connection, message_handler, arg);
  ...
  admin_unpair_connection->admin_unpair_step = BLE_ADMIN_UNPAIR_BEGIN;
...
ADMIN_ERROR_EXIT:
  ...
  if (admin_unpair_connection->gatt_connection)
  {
    gattlib_notification_stop(
      admin_unpair_connection->gatt_connection, &(admin_unpair_connection->admin_uuid));
    gattlib_disconnect(admin_unpair_connection->gatt_connection);
  }
...
// --------------------------------------------------------
int bleInitAdminUnpairParam(ble_admin_unpair_param_t *admin_unpair_param)
{
  memset(admin_unpair_param, 0, sizeof(ble_admin_unpair_param_t));
  return 0;
}

int bleReleaseAdminUnpairParam(ble_admin_unpair_param_t *admin_unpair_param)
{
  if (admin_unpair_param->lock)
  {
    free(admin_unpair_param->lock);
    admin_unpair_param->lock = NULL;
  }
  return 0;
}

int bleSetAdminUnpairParam(ble_admin_unpair_param_t *admin_unpair_param, igm_lock_t *lock)
{
  bleReleaseAdminUnpairParam(admin_unpair_param);
  admin_unpair_param->lock = calloc(sizeof(igm_lock_t), 1);
  lockCopy(admin_unpair_param->lock, lock);
  return 0;
}
...
{
  memset(result, 0, sizeof(ble_admin_unpair_result_t));
  return 0;
}
...
{
  return result->unpair_successed;
}
