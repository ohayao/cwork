...
int testGetLockBatteryLevel(igm_lock_t *lock) {
    serverLog(LL_NOTICE,"get lock battery level cmd ask invoker to release the lock.");
    ...
    fsm_table_t *get_battery_level_status_fsm = getAdminGetBatteryLevelFsmTable();
    ...
    tn->task_sm_table = get_battery_level_status_fsm;

    tn->task_type = TASK_BLE_ADMIN_GET_BATTERY_LEVEL;
    serverLog(LL_NOTICE, "fsm_max_n: %d", fsm_max_n);
...
int main(int argc, char *argv[]) {
    ...
    int res = testGetLockBatteryLevel(lock);
    releaseLock(&lock);
    return 0;
}
