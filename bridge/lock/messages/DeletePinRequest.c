      ...
      switch (tag_id) {
        case 0:
          if(value_type == CborIntegerType){
            ...
            //msgId value
            if(val != 47) return IgSerializerErrorInvalidMsgIdValue;
            err = cbor_value_advance_fixed(&content);
            break;
          }
        ...
        case 11:
            if(value_type == CborByteStringType){
                ...
                ig_DeletePinRequest_set_password_nocopy(retval, data_arr, size);
                break;
            }
        ...
        case 12:
            if(value_type == CborByteStringType){
                ...
                ig_DeletePinRequest_set_old_pin_nocopy(retval, data_arr, size);
                break;
            }
        ...
        case 100:
            if(value_type == CborIntegerType){
                ...
                ig_DeletePinRequest_set_operation_id(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
...
{
  return 33 + obj->password_size + obj->old_pin_size; //13 + 20;
}
