...
IgSerializerError ig_NbStatusNotification_encode(IgNbStatusNotification *obj,uint8_t *retval,uint32_t length,size_t *written_length)
{
  ...
  if(!ig_NbStatusNotification_is_valid(obj)) return IgSerializerErrorInvalidPayload;
      ...
      switch (tag_id) {
        case 0:
          if(value_type == CborIntegerType){
            ...
            //msgId value
            if(val != 105) return IgSerializerErrorInvalidMsgIdValue;
            err = cbor_value_advance_fixed(&content);
            break;
          }
        ...
        case 11:
            if(value_type == CborIntegerType){
                ...
                ig_NbStatusNotification_set_nb_network_status(retval, (uint8_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
