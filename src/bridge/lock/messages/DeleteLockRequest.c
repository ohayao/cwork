...
IgSerializerError ig_DeleteLockRequest_encode(IgDeleteLockRequest *obj,uint8_t *retval,uint32_t length,size_t *written_length)
{
  ...
  if(!ig_DeleteLockRequest_is_valid(obj)) return IgSerializerErrorInvalidPayload;
      ...
      switch (tag_id) {
        case 0:
          if(value_type == CborIntegerType){
            ...
            //msgId value
            if(val != 85) return IgSerializerErrorInvalidMsgIdValue;
            err = cbor_value_advance_fixed(&content);
            break;
          }
        ...
        case 11:
            if(value_type == CborByteStringType){
                ...
                ig_DeleteLockRequest_set_password_nocopy(retval, data_arr, size);
                break;
            }
        ...
        case 12:
            if(value_type == CborByteStringType){
                ...
                ig_DeleteLockRequest_set_lock_id_nocopy(retval, data_arr, size);
                break;
            }
        ...
        case 100:
            if(value_type == CborIntegerType){
                ...
                ig_DeleteLockRequest_set_operation_id(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
...
{
  return 33 + obj->password_size + obj->lock_id_size; //13 + 20;
}
