      ...
      switch (tag_id) {
        case 0:
          if(value_type == CborIntegerType){
            ...
            //msgId value
            if(val != 27) return IgSerializerErrorInvalidMsgIdValue;
            err = cbor_value_advance_fixed(&content);
            break;
          }
        ...
        case 11:
            if(value_type == CborByteStringType){
                ...
                ig_GetTimeRequest_set_password_nocopy(retval, data_arr, size);
                break;
            }
        ...
        case 100:
            if(value_type == CborIntegerType){
                ...
                ig_GetTimeRequest_set_operation_id(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
