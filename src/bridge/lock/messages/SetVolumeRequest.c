...
IgSerializerError ig_SetVolumeRequest_encode(IgSetVolumeRequest *obj,uint8_t *retval,uint32_t length,size_t *written_length)
{
  ...
  if(err) return (IgSerializerError) err;
      ...
      if(err) return (IgSerializerError) err;
      err = cbor_encode_uint(&map, obj->volume);
      if(err) return (IgSerializerError) err;
      ...
      switch (tag_id) {
        case 0:
          if(value_type == CborIntegerType){
            ...
            //msgId value
            if(val != 33) return IgSerializerErrorInvalidMsgIdValue;
            err = cbor_value_advance_fixed(&content);
            break;
          }
        ...
        case 11:
            if(value_type == CborByteStringType){
                ...
                ig_SetVolumeRequest_set_password_nocopy(retval, data_arr, size);
                break;
            }
        ...
        case 12:
            if(value_type == CborIntegerType){
                ...
                ig_SetVolumeRequest_set_volume(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
        ...
        case 100:
            if(value_type == CborIntegerType){
                uint64_t val;
                cbor_value_get_uint64(&content, &val);
                ig_SetVolumeRequest_set_operation_id(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
...
{
  return obj->has_password && obj->has_volume;
}
