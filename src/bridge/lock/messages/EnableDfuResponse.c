...
IgSerializerError ig_EnableDfuResponse_encode(IgEnableDfuResponse *obj,uint8_t *retval,uint32_t length,size_t *written_length)
{
  ...
  if(!ig_EnableDfuResponse_is_valid(obj)) return IgSerializerErrorInvalidPayload;
      ...
      switch (tag_id) {
        case 0:
          if(value_type == CborIntegerType){
            ...
            //msgId value
            if(val != 62) return IgSerializerErrorInvalidMsgIdValue;
            err = cbor_value_advance_fixed(&content);
            break;
          }
        ...
        case 11:
            if(value_type == CborIntegerType){
                ...
                ig_EnableDfuResponse_set_result(retval, (uint8_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
        ...
        case 100:
            if(value_type == CborIntegerType){
                uint64_t val;
                cbor_value_get_uint64(&content, &val);
                ig_EnableDfuResponse_set_operation_id(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
