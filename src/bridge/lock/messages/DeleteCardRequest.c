...
{
  memset(obj, 0, sizeof(IgDeleteCardRequest));
}
IgSerializerError ig_DeleteCardRequest_encode(IgDeleteCardRequest *obj,uint8_t *retval,uint32_t length,size_t *written_length)
{
  ...
  if(!ig_DeleteCardRequest_is_valid(obj)) return IgSerializerErrorInvalidPayload;
  ...
  if(obj->has_duration) fields_size++;
  ...
  if(obj->has_duration){
      ...
      if(err) return (IgSerializerError) err;
      err = cbor_encode_uint(&map, obj->duration);
      if(err) return (IgSerializerError) err;
  }
      ...
      switch (tag_id) {
        case 0:
          if(value_type == CborIntegerType){
            ...
            //msgId value
            if(val != 67) return IgSerializerErrorInvalidMsgIdValue;
            err = cbor_value_advance_fixed(&content);
            break;
          }
        ...
        case 11:
            if(value_type == CborByteStringType){
                ...
                ig_DeleteCardRequest_set_password_nocopy(retval, data_arr, size);
                break;
            }
        ...
        case 12:
            if(value_type == CborIntegerType){
                ...
                ig_DeleteCardRequest_set_duration(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
        ...
        case 100:
            if(value_type == CborIntegerType){
                uint64_t val;
                cbor_value_get_uint64(&content, &val);
                ig_DeleteCardRequest_set_operation_id(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
