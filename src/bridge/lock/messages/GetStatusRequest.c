      ...
      switch (tag_id) {
        ...
        case 11:
            if(value_type == CborByteStringType){
                ...
                ig_GetStatusRequest_set_password_nocopy(retval, data_arr, size);
                break;
            }
        ...
        case 100:
            if(value_type == CborIntegerType){
                ...
                ig_GetStatusRequest_set_operation_id(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
