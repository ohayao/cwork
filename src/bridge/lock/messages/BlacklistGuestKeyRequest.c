...
IgSerializerError ig_BlacklistGuestKeyRequest_encode(IgBlacklistGuestKeyRequest *obj,uint8_t *retval,uint32_t length,size_t *written_length)
{
  ...
  if(!ig_BlacklistGuestKeyRequest_is_valid(obj)) return IgSerializerErrorInvalidPayload;
      ...
      switch (tag_id) {
        case 0:
          if(value_type == CborIntegerType){
            ...
            //msgId value
            if(val != 57) return IgSerializerErrorInvalidMsgIdValue;
            err = cbor_value_advance_fixed(&content);
            break;
          }
        ...
        case 11:
            if(value_type == CborByteStringType){
                ...
                ig_BlacklistGuestKeyRequest_set_password_nocopy(retval, data_arr, size);
                break;
            }
        ...
        case 12:
            if(value_type == CborIntegerType){
                ...
                ig_BlacklistGuestKeyRequest_set_key_id(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
        ...
        case 13:
            if(value_type == CborIntegerType){
                uint64_t val;
                cbor_value_get_uint64(&content, &val);
                ig_BlacklistGuestKeyRequest_set_start_date(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
        ...
        case 14:
            if(value_type == CborIntegerType){
                uint64_t val;
                cbor_value_get_uint64(&content, &val);
                ig_BlacklistGuestKeyRequest_set_end_date(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
        ...
        case 100:
            if(value_type == CborIntegerType){
                uint64_t val;
                cbor_value_get_uint64(&content, &val);
                ig_BlacklistGuestKeyRequest_set_operation_id(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
...
{
  return 44 + obj->password_size; //13 + 31;
}
