...
IgSerializerError ig_SetRelockAlarmRequest_encode(IgSetRelockAlarmRequest *obj,uint8_t *retval,uint32_t length,size_t *written_length)
{
  ...
  if(!ig_SetRelockAlarmRequest_is_valid(obj)) return IgSerializerErrorInvalidPayload;
  ...
  if(obj->has_relock_alarm_seconds) fields_size++;
  ...
  if(obj->has_relock_alarm_seconds){
      err = cbor_encode_uint(&map, 12);
      if(err) return (IgSerializerError) err;
      err = cbor_encode_uint(&map, obj->relock_alarm_seconds);
      if(err) return (IgSerializerError) err;
  }
      ...
      switch (tag_id) {
        case 0:
          if(value_type == CborIntegerType){
            ...
            //msgId value
            if(val != 73) return IgSerializerErrorInvalidMsgIdValue;
            err = cbor_value_advance_fixed(&content);
            break;
          }
        ...
        case 11:
            if(value_type == CborByteStringType){
                ...
                ig_SetRelockAlarmRequest_set_password_nocopy(retval, data_arr, size);
                break;
            }
        ...
        case 12:
            if(value_type == CborIntegerType){
                ...
                ig_SetRelockAlarmRequest_set_relock_alarm_seconds(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
        ...
        case 13:
            if(value_type == CborBooleanType){
                ...
                ig_SetRelockAlarmRequest_set_enable(retval, val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
        ...
        case 100:
            if(value_type == CborIntegerType){
                uint64_t val;
                cbor_value_get_uint64(&content, &val);
                ig_SetRelockAlarmRequest_set_operation_id(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
...
{
  obj->relock_alarm_seconds = relock_alarm_seconds;
  obj->has_relock_alarm_seconds = true;
}
