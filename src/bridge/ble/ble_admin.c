...
static int write_unpair_request(void *arg)
{
	...
	free(encryptPayloadBytes);
	admin_unpair_connection->admin_step = BLE_ADMIN_UNPAIR_REQUEST;
	return 0;
...
int register_admin_notfication(void *arg)
{
	...
	serverLog(LL_NOTICE, "@@@ gattlib_string_to_uuid to admin_uuid[%u] success.", admin_connection->admin_uuid);
