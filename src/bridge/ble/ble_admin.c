...
void message_handler(const uuid_t* uuid, const uint8_t* data, size_t data_length, void* user_data) {
	...
	switch(admin_connection->admin_step)
	{
		...
		case BLE_ADMIN_GETBATTERYLEVEL_REQUEST:
			{
				serverLog(LL_NOTICE, "BLE_ADMIN_GETBATTERYLEVEL_REQUEST handleGetBatteryLevelResponce");
...
fsm_table_t admin_getlogs_fsm_table[ADMIN_GET_LOGS_SM_TABLE_LEN] = {
  ...
  {BLE_ADMIN_STEP1,         waiting_admin_step1,          BLE_ADMIN_STEP2},
  {BLE_ADMIN_STEP2,         write_admin_step2,            BLE_ADMIN_ESTABLISHED},
  {BLE_ADMIN_ESTABLISHED,   waiting_admin_step3,          BLE_ADMIN_GETLOGS_REQUEST},
  {BLE_ADMIN_GETLOGS_REQUEST,   writeGetLogsRequest,      BLE_ADMIN_GETLOGS_RESULT},
  {BLE_ADMIN_GETLOGS_RESULT,  waitingGetLogsResult,        BLE_ADMIN_GETLOGS_DONE},
};
...
static int handleGetLogsResponce(const uint8_t* data, int data_length,void* user_data) {
	...
	if (admin_connection->step_max_size == admin_connection->step_cur_size) { 
		...
		IgSerializerError err = ig_GetLogsResponse_decode(responceBytes, responceLen, &admin_getlogs_responce, 0);
		if (err) {
			serverLog(LL_NOTICE, "ig_GetLogsResponse_decode err %d", err);
			admin_connection->receive_err = 1;
			goto GETLOGS_RESPONCE_EXIT;
		}
		serverLog(LL_NOTICE, "get log info has_result[%d], result[%d], has_data[%d], data_size[%d]", admin_getlogs_responce.has_result, admin_getlogs_responce.result, admin_getlogs_responce.has_data, admin_getlogs_responce.data_size);
...
static int writeGetBatteryLevelRequest(void *arg) {
	...
	ig_GetBatteryLevelRequest_set_password( &request, admin_connection->lock->password, admin_connection->lock->password_size);
...
GET_BATTERY_ERROR:
	...
	setAdminResultBatteryRequestErr(admin_connection->ble_result, 1);
	bleSetBleResult(ble_data, admin_connection->ble_result);
	ret = releaseAdminConnection(&admin_connection);
	if (ret) {
		serverLog(LL_ERROR, "releaseAdminConnection error");
...
static int handleGetBatteryLevelResponce(const uint8_t* data, int data_length,void* user_data)
{
	...
	if (admin_connection->step_max_size == admin_connection->step_cur_size) {
		...
		admin_connection->admin_step = BLE_ADMIN_GETBATTERYLEVEL_RESULT;
		...
		if (responce.has_result) {
			...
			admin_connection->ble_result->get_battery_level_result = responce.result;
		}
		...
		// 返回参数给调用进程
		serverLog(LL_NOTICE, "handle_step3_message bleSetBleResult to ble data");
		setAdminResultCMDResponse( admin_connection->ble_result, &responce, sizeof(IgDeletePinResponse));
		bleSetBleResult(ble_data, admin_connection->ble_result);
...
void setAdminResultBatteryRequestErr(ble_admin_result_t *result, int err) {
