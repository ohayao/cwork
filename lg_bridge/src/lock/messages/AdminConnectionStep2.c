...
IgSerializerError ig_AdminConnectionStep2_encode(IgAdminConnectionStep2 *obj,uint8_t *retval,uint32_t length,size_t *written_length)
{
  ...
  if(!ig_AdminConnectionStep2_is_valid(obj)) return IgSerializerErrorInvalidPayload;
      ...
      switch (tag_id) {
        case 0:
          if(value_type == CborIntegerType){
            ...
            //msgId value
            if(val != 7) return IgSerializerErrorInvalidMsgIdValue;
            err = cbor_value_advance_fixed(&content);
            break;
          }
        ...
        case 11:
            if(value_type == CborByteStringType){
                ...
                ig_AdminConnectionStep2_set_nonce_nocopy(retval, data_arr, size);
                break;
            }
          default:
