...
void *ble_write_pairing_commit(void *arg)
{
	...
	else
	{
		...
		sp_lock_list->push_front(
			paired_igm_lock_t(connection->lock, true)
		);
		...
		sp_lock_list->push_back(paired_igm_lock_t(connection->lock, true));
...
gboolean stop_pair_func(gpointer data)
{
...
void *ble_pair_device(void *arg) {
  ...
  else
  {
	...
	// g_timeout_add_seconds (DEFAULT_PAIR_TIMEOUT, stop_pair_func, loop);
	g_main_loop_run(loop);
	// 在配对借结束, 会quit, 所以这里不需要了
