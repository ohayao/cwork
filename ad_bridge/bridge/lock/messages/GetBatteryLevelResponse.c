...
IgSerializerError ig_GetBatteryLevelResponse_encode(IgGetBatteryLevelResponse *obj,uint8_t *retval,uint32_t length,size_t *written_length)
{
  ...
  if(!ig_GetBatteryLevelResponse_is_valid(obj)) return IgSerializerErrorInvalidPayload;
  ...
  if(obj->has_battery_level) fields_size++;
  ...
  if(obj->has_battery_level){
      err = cbor_encode_uint(&map, 12);
      if(err) return (IgSerializerError) err;
      err = cbor_encode_uint(&map, obj->battery_level);
      if(err) return (IgSerializerError) err;
  }
      ...
      switch (tag_id) {
        case 0:
          if(value_type == CborIntegerType){
            ...
            //msgId value
            if(val != 30) return IgSerializerErrorInvalidMsgIdValue;
            err = cbor_value_advance_fixed(&content);
            break;
          }
        ...
        case 11:
            if(value_type == CborIntegerType){
                ...
                ig_GetBatteryLevelResponse_set_result(retval, (uint8_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
        ...
        case 12:
            if(value_type == CborIntegerType){
                uint64_t val;
                cbor_value_get_uint64(&content, &val);
                ig_GetBatteryLevelResponse_set_battery_level(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
        ...
        case 100:
            if(value_type == CborIntegerType){
                uint64_t val;
                cbor_value_get_uint64(&content, &val);
                ig_GetBatteryLevelResponse_set_operation_id(retval, (uint32_t) val);
                err = cbor_value_advance_fixed(&content);
                if(err) return (IgSerializerError) err;
                break;
            }
...
{
  obj->battery_level = battery_level;
  obj->has_battery_level = true;
}
